...
const Register = () => {
    const [formData, setFormData] = useState({ name: '', email: '', password: '', role: 'user', phone: '', address: '', city: '', state: '', zip: '' });
        ...
        <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
            <div className="max-w-md w-full bg-white p-8 rounded-xl shadow-lg border">
                ...
                <form onSubmit={handleSubmit}>
                    <RegisterFields formData={formData} handleChange={(e) => setFormData({ ...formData, [e.target.name]: e.target.value })} />
                    <LocationFields handleChange={(e) => setFormData({ ...formData, [e.target.name]: e.target.value })} />
                    <button type="submit" className="w-full bg-blue-600 text-white mt-6 py-3 rounded-lg font-bold">Register</button>
                    <div className="text-center mt-4"><Link to="/login" className="text-blue-600 font-semibold">Already have an account? Login</Link></div>
                </form>
            </div>
        </div>
    );
